...
img = cv2.imread("2.JPG")  # Replace with your image
...
# --- Smart background removal using saturation + edge ---
s_channel = hsv_blurred[:, :, 1]
sat_mask = cv2.inRange(s_channel, 0, 60)  # low saturation = likely background

edges = cv2.Canny(gray, 50, 150)
edges_dilated = cv2.dilate(edges, np.ones((3, 3), np.uint8), iterations=1)

background_mask = cv2.bitwise_and(sat_mask, cv2.bitwise_not(edges_dilated))
...
titles = [
    "Original", "Saturation Mask", "Leaf Mask",
